...
export default function SignIn({ onSuccess }: SignInProps) {
  ...
  useEffect(() => {
    // Check if redirect has query registered parameter
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/auth/signin', {
        ...
        credentials: 'include', // Required: lets browser store the httpOnly auth cookie
      ...
      <div className="max-w-[420px] w-full bg-white rounded-2xl p-8 border border-gray-100/80 shadow-md shadow-gray-100/50 space-y-6">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-gray-500 font-medium">
            Sign in to start ordering fresh gourmet meals
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="text-right">
            <button
              ...
              className="text-xs text-[#e53e3e] font-semibold hover:underline focus:outline-none"
            >
              Forgot Password?
            </button>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="text-center">
            <span className="text-xs text-gray-500 font-medium">
              ...
              <Link to="/signup" className="text-[#e53e3e] font-bold hover:underline">
                Sign Up
              </Link>
            </span>
          </div>
